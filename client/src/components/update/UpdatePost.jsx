...
export default function UpdatePost({ post, userId, setIsOpen }) {
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async ({ updatedText, imgFile }) => {
      ...
      await queryClient.cancelQueries(["posts", "feed"]);
      ...
      // Initialize optimistic image and temporary Blob URL for cleanup
      let optimisticCacheImg = post.img; // Default image in DB
      ...
      // 2. If imgFile is null, optimisticCacheImg remains post.img (the existing image)

      // Optimistically update both global and user's posts caches
      const applyOptimisticUpdate = (oldPosts = []) =>
        oldPosts.map((p) =>
          // Use optimisticCacheImg which is either the Blob URL or the existing image
          p.id === post.id
            ? { ...p, text: updatedText, img: optimisticCacheImg }
            : p
        );

      queryClient.setQueryData(["posts", "feed"], applyOptimisticUpdate);
